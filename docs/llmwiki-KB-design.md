...
flowchart TB
    subgraph Capture["Capture Layer"]
        ...
        WebClip["Web Clipper / Browser"]
    end

    subgraph Storage["Storage Layer"]
        Raw["raw/  (Object Storage)<br/>S3 / R2 / GCS<br/>Immutable originals + extracts"]
        Wiki["wiki/  (Object Storage or Git)<br/>Compiled markdown<br/>indexes, concepts, entities"]
    end

    subgraph Processing["Processing Layer"]
        IngestPipe["Ingestion Pipeline<br/>(Serverless / Queue)"]
        Extract["Extract & Normalize<br/>PDF · Web · YT · Image"]
        Embed["Embedding + Chunking"]
        Compile["Incremental Wiki Compiler"]
    end

    subgraph Retrieval["Retrieval & Agent Layer"]
        Vector["Vector Store<br/>(S3 Vectors / Vectorize / Qdrant / pgvector)"]
        Agent["Tool-using Agent<br/>Wiki-first + RAG fallback"]
        LocalLLM["Optional Local LLM<br/>(Ollama / vLLM)"]
        CloudLLM["Cloud LLM<br/>(Haiku / Flash / mini + stronger)"]
    end
    ...
    subgraph Interface["Interface Layer"]
        ...
        CorePkg["Python Core Package"]
    end

    Capture --> Raw
    Raw --> IngestPipe
    IngestPipe --> Extract --> Embed --> Vector
    Extract --> Compile
    Compile --> Wiki
    Wiki --> Agent
    Vector --> Agent
...
1. **Core logic as a clean Python package**  
...
2. **Expose primarily through FastAPI**  
...
3. **Add an MCP server layer on top** (FastMCP or equivalent)  
